...
from dsptype import BuildTool_Path, CircleGizmo, Configs, Maths, GameGizmo
from dspharmony import HarmonyPatch_Postfix_Default
...
def Start():
    SetTeleportGizmo()
    HarmonyPatch_Postfix_Default(BuildTool_Path, "UpdateRaycast", postfunc=Postfix) # void UpdateRaycast()
...
def CreatCircleGizmo(position, texid, radius, percent):
    ...
    gameObject = 创建对象and复制网格(Configs.builtin.circleGizmoPrefab.gameObject)
...
# def 复制对象(fromObject, parentTransform): # 存在 ...get_transform() 为null的bug
...
def 创建对象and复制网格(fromObject, parentTransform=None):
...
# Uinty不允许多线程，直接操作主线程中的游戏对象数据，如Transform
# 多线程中，使用主线程调度器（MainThreadDispatcher）来确保在主线程中访问 Unity 对象。
# MainThreadDispatcher 
# mainThreadDispatcher = gameObject.AddComponent[MainThreadDispatcher]()
# antion = Action[](function)
# 
# mainThreadDispatcher.Enqueue(action)

# def function():
    # cube = GameObject.Find("Cube")
    # cube.transform.position = Vector3(0.0, 0.0, 0.0)